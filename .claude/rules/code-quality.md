...
- Files and directories: kebab-case (`servicenow-client.ts`, `table-api.ts`). One exported class per file uses the same kebab name.
- Booleans: `is` / `has` / `should` / `can` prefix. Functions: verb-first (`fetchIncident`). Factories: `create*`. Converters: `to*`. Predicates: `is*` / `has*`. Constants: `SCREAMING_SNAKE`.
- Abbreviations only when universally known (`id`, `url`, `api`, `env`). Acronyms as words: `sysId`, not `sysID`. ServiceNow-specific: `sysId`, `sysparmQuery`, `tableName`.
