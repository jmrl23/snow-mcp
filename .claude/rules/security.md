...
- ServiceNow credentials live in `.env` (`SNOW_INSTANCE_URL`, `SNOW_USER`, `SNOW_PASSWORD` / `SNOW_OAUTH_TOKEN`). Read them only via `process.env`. Never hardcode, never log, never include in error messages or thrown errors.
